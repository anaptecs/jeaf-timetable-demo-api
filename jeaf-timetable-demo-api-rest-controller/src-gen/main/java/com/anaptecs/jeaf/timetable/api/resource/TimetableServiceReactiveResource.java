...
import com.anaptecs.jeaf.timetable.api.TimetableServiceReactive;
...
@RestController
public class TimetableServiceReactiveResource {
  /**
  ...
  /**
   * All request to this class will be delegated to {@link com.anaptecs.jeaf.timetable.api.TimetableService}.
   */
  private final TimetableServiceReactive timetableService;

  /**
   * Initialize object.
   *
   * @param pTimetableService Dependency on concrete {@link com.anaptecs.jeaf.timetable.api.TimetableService}
   * implementation that should be used.
   * @param pValidationExecutor Validation executor for request / response validation.
   */
  public TimetableServiceReactiveResource( TimetableServiceReactive pTimetableService,
  ...
  /**
   * {@link com.anaptecs.jeaf.timetable.api.TimetableService#executeTripSearch()}
   */
  @ResponseStatus(HttpStatus.OK)
  @RequestMapping(path = "trips", method = { RequestMethod.GET })
  public Mono<TimetableResponse> executeTripSearch(
      ...
      @RequestHeader(name = "Time-Shift", required = false) String pTimeShift, ServerWebExchange pServerWebExchange ) {
    ...
    return Mono.defer(( ) -> {
      ...
      validationExecutor.validateRequest(TimetableServiceReactive.class, request, pContext);
      // Delegate request to service.
      return timetableService.executeTripSearch(request, pContext);
    }).doOnNext(lResponse ->
    // Validate response.
    validationExecutor.validateResponse(TimetableServiceReactive.class, lResponse));
  }

  /**
   * {@link com.anaptecs.jeaf.timetable.api.TimetableService#executeIntervalSearch()}
   */
  @ResponseStatus(HttpStatus.OK)
  @RequestMapping(path = "interval", method = { RequestMethod.GET })
  public Mono<TimetableResponse> executeIntervalSearch(
      @RequestParam(name = "origin", required = true) int pOriginAsBasicType,
      @RequestParam(name = "destination", required = true) int pDestinationAsBasicType,
      @RequestParam(name = "vias", required = false) int[] pViasAsBasicType,
      @RequestParam(name = "start", required = true) String pStartAsBasicType,
    ...
    return Mono.defer(( ) -> {
      // Validate request parameter(s).
      validationExecutor.validateRequest(TimetableServiceReactive.class, pRequest, pContext);
      // Delegate request to service.
      return timetableService.executeIntervalSearch(pRequest, pContext);
    }).doOnNext(lResponse ->
    // Validate response.
    validationExecutor.validateResponse(TimetableServiceReactive.class, lResponse));
  }
